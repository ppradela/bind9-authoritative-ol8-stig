...
# the source context (typically default_t) instead of named_zone_t and
# named will fail to load the zones despite correct Unix ownership.
...
- `CAP_SETUID` / `CAP_SETGID` — root can no longer drop to user `named`, so `named -u named` would fail too.
